// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.27.1
// 	protoc        v3.6.1
// source: strategynode.proto

package strategynode

import (
	common "github.com/lendloan/lendproto/common"
	rescode "github.com/lendloan/lendproto/rescode"
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	reflect "reflect"
	sync "sync"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

type QueryReq struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	Auth *common.Authorize `protobuf:"bytes,1,opt,name=auth,proto3" json:"auth,omitempty"`
	Uid  int64             `protobuf:"varint,2,opt,name=uid,proto3" json:"uid,omitempty"`
	Info map[string]string `protobuf:"bytes,3,rep,name=info,proto3" json:"info,omitempty" protobuf_key:"bytes,1,opt,name=key,proto3" protobuf_val:"bytes,2,opt,name=value,proto3"`
}

func (x *QueryReq) Reset() {
	*x = QueryReq{}
	if protoimpl.UnsafeEnabled {
		mi := &file_strategynode_proto_msgTypes[0]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *QueryReq) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*QueryReq) ProtoMessage() {}

func (x *QueryReq) ProtoReflect() protoreflect.Message {
	mi := &file_strategynode_proto_msgTypes[0]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use QueryReq.ProtoReflect.Descriptor instead.
func (*QueryReq) Descriptor() ([]byte, []int) {
	return file_strategynode_proto_rawDescGZIP(), []int{0}
}

func (x *QueryReq) GetAuth() *common.Authorize {
	if x != nil {
		return x.Auth
	}
	return nil
}

func (x *QueryReq) GetUid() int64 {
	if x != nil {
		return x.Uid
	}
	return 0
}

func (x *QueryReq) GetInfo() map[string]string {
	if x != nil {
		return x.Info
	}
	return nil
}

type QueryRes struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	Rescode  rescode.Code       `protobuf:"varint,1,opt,name=rescode,proto3,enum=rescode.Code" json:"rescode,omitempty"`
	Resmsg   string             `protobuf:"bytes,2,opt,name=resmsg,proto3" json:"resmsg,omitempty"`
	Strategy []*common.Strategy `protobuf:"bytes,3,rep,name=strategy,proto3" json:"strategy,omitempty"`
	Extra    map[string]string  `protobuf:"bytes,4,rep,name=extra,proto3" json:"extra,omitempty" protobuf_key:"bytes,1,opt,name=key,proto3" protobuf_val:"bytes,2,opt,name=value,proto3"`
}

func (x *QueryRes) Reset() {
	*x = QueryRes{}
	if protoimpl.UnsafeEnabled {
		mi := &file_strategynode_proto_msgTypes[1]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *QueryRes) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*QueryRes) ProtoMessage() {}

func (x *QueryRes) ProtoReflect() protoreflect.Message {
	mi := &file_strategynode_proto_msgTypes[1]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use QueryRes.ProtoReflect.Descriptor instead.
func (*QueryRes) Descriptor() ([]byte, []int) {
	return file_strategynode_proto_rawDescGZIP(), []int{1}
}

func (x *QueryRes) GetRescode() rescode.Code {
	if x != nil {
		return x.Rescode
	}
	return rescode.Code(0)
}

func (x *QueryRes) GetResmsg() string {
	if x != nil {
		return x.Resmsg
	}
	return ""
}

func (x *QueryRes) GetStrategy() []*common.Strategy {
	if x != nil {
		return x.Strategy
	}
	return nil
}

func (x *QueryRes) GetExtra() map[string]string {
	if x != nil {
		return x.Extra
	}
	return nil
}

type VerifyReq struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	Auth *common.Authorize `protobuf:"bytes,1,opt,name=auth,proto3" json:"auth,omitempty"`
	Uid  int64             `protobuf:"varint,2,opt,name=uid,proto3" json:"uid,omitempty"`
	Info map[string]string `protobuf:"bytes,3,rep,name=info,proto3" json:"info,omitempty" protobuf_key:"bytes,1,opt,name=key,proto3" protobuf_val:"bytes,2,opt,name=value,proto3"`
}

func (x *VerifyReq) Reset() {
	*x = VerifyReq{}
	if protoimpl.UnsafeEnabled {
		mi := &file_strategynode_proto_msgTypes[2]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *VerifyReq) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*VerifyReq) ProtoMessage() {}

func (x *VerifyReq) ProtoReflect() protoreflect.Message {
	mi := &file_strategynode_proto_msgTypes[2]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use VerifyReq.ProtoReflect.Descriptor instead.
func (*VerifyReq) Descriptor() ([]byte, []int) {
	return file_strategynode_proto_rawDescGZIP(), []int{2}
}

func (x *VerifyReq) GetAuth() *common.Authorize {
	if x != nil {
		return x.Auth
	}
	return nil
}

func (x *VerifyReq) GetUid() int64 {
	if x != nil {
		return x.Uid
	}
	return 0
}

func (x *VerifyReq) GetInfo() map[string]string {
	if x != nil {
		return x.Info
	}
	return nil
}

type VerifyRes struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	Rescode rescode.Code      `protobuf:"varint,1,opt,name=rescode,proto3,enum=rescode.Code" json:"rescode,omitempty"`
	Resmsg  string            `protobuf:"bytes,2,opt,name=resmsg,proto3" json:"resmsg,omitempty"`
	Next    *common.Strategy  `protobuf:"bytes,3,opt,name=next,proto3" json:"next,omitempty"`
	Extra   map[string]string `protobuf:"bytes,4,rep,name=extra,proto3" json:"extra,omitempty" protobuf_key:"bytes,1,opt,name=key,proto3" protobuf_val:"bytes,2,opt,name=value,proto3"`
}

func (x *VerifyRes) Reset() {
	*x = VerifyRes{}
	if protoimpl.UnsafeEnabled {
		mi := &file_strategynode_proto_msgTypes[3]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *VerifyRes) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*VerifyRes) ProtoMessage() {}

func (x *VerifyRes) ProtoReflect() protoreflect.Message {
	mi := &file_strategynode_proto_msgTypes[3]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use VerifyRes.ProtoReflect.Descriptor instead.
func (*VerifyRes) Descriptor() ([]byte, []int) {
	return file_strategynode_proto_rawDescGZIP(), []int{3}
}

func (x *VerifyRes) GetRescode() rescode.Code {
	if x != nil {
		return x.Rescode
	}
	return rescode.Code(0)
}

func (x *VerifyRes) GetResmsg() string {
	if x != nil {
		return x.Resmsg
	}
	return ""
}

func (x *VerifyRes) GetNext() *common.Strategy {
	if x != nil {
		return x.Next
	}
	return nil
}

func (x *VerifyRes) GetExtra() map[string]string {
	if x != nil {
		return x.Extra
	}
	return nil
}

var File_strategynode_proto protoreflect.FileDescriptor

var file_strategynode_proto_rawDesc = []byte{
	,
}

var (
	file_strategynode_proto_rawDescOnce sync.Once
	file_strategynode_proto_rawDescData = file_strategynode_proto_rawDesc
)

func file_strategynode_proto_rawDescGZIP() []byte {
	file_strategynode_proto_rawDescOnce.Do(func() {
		file_strategynode_proto_rawDescData = protoimpl.X.CompressGZIP(file_strategynode_proto_rawDescData)
	})
	return file_strategynode_proto_rawDescData
}

var file_strategynode_proto_msgTypes = make([]protoimpl.MessageInfo, 8)
var file_strategynode_proto_goTypes = []interface{}{
	(*QueryReq)(nil),         // 0: strategynode.QueryReq
	(*QueryRes)(nil),         // 1: strategynode.QueryRes
	(*VerifyReq)(nil),        // 2: strategynode.VerifyReq
	(*VerifyRes)(nil),        // 3: strategynode.VerifyRes
	nil,                      // 4: strategynode.QueryReq.InfoEntry
	nil,                      // 5: strategynode.QueryRes.ExtraEntry
	nil,                      // 6: strategynode.VerifyReq.InfoEntry
	nil,                      // 7: strategynode.VerifyRes.ExtraEntry
	(*common.Authorize)(nil), // 8: common.Authorize
	(rescode.Code)(0),        // 9: rescode.Code
	(*common.Strategy)(nil),  // 10: common.Strategy
}
var file_strategynode_proto_depIdxs = []int32{
	8,  // 0: strategynode.QueryReq.auth:type_name -> common.Authorize
	4,  // 1: strategynode.QueryReq.info:type_name -> strategynode.QueryReq.InfoEntry
	9,  // 2: strategynode.QueryRes.rescode:type_name -> rescode.Code
	10, // 3: strategynode.QueryRes.strategy:type_name -> common.Strategy
	5,  // 4: strategynode.QueryRes.extra:type_name -> strategynode.QueryRes.ExtraEntry
	8,  // 5: strategynode.VerifyReq.auth:type_name -> common.Authorize
	6,  // 6: strategynode.VerifyReq.info:type_name -> strategynode.VerifyReq.InfoEntry
	9,  // 7: strategynode.VerifyRes.rescode:type_name -> rescode.Code
	10, // 8: strategynode.VerifyRes.next:type_name -> common.Strategy
	7,  // 9: strategynode.VerifyRes.extra:type_name -> strategynode.VerifyRes.ExtraEntry
	0,  // 10: strategynode.StrategynodeService.Query:input_type -> strategynode.QueryReq
	2,  // 11: strategynode.StrategynodeService.Verify:input_type -> strategynode.VerifyReq
	1,  // 12: strategynode.StrategynodeService.Query:output_type -> strategynode.QueryRes
	3,  // 13: strategynode.StrategynodeService.Verify:output_type -> strategynode.VerifyRes
	12, // [12:14] is the sub-list for method output_type
	10, // [10:12] is the sub-list for method input_type
	10, // [10:10] is the sub-list for extension type_name
	10, // [10:10] is the sub-list for extension extendee
	0,  // [0:10] is the sub-list for field type_name
}

func init() { file_strategynode_proto_init() }
func file_strategynode_proto_init() {
	if File_strategynode_proto != nil {
		return
	}
	if !protoimpl.UnsafeEnabled {
		file_strategynode_proto_msgTypes[0].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*QueryReq); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_strategynode_proto_msgTypes[1].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*QueryRes); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_strategynode_proto_msgTypes[2].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*VerifyReq); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_strategynode_proto_msgTypes[3].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*VerifyRes); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
	}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: file_strategynode_proto_rawDesc,
			NumEnums:      0,
			NumMessages:   8,
			NumExtensions: 0,
			NumServices:   1,
		},
		GoTypes:           file_strategynode_proto_goTypes,
		DependencyIndexes: file_strategynode_proto_depIdxs,
		MessageInfos:      file_strategynode_proto_msgTypes,
	}.Build()
	File_strategynode_proto = out.File
	file_strategynode_proto_rawDesc = nil
	file_strategynode_proto_goTypes = nil
	file_strategynode_proto_depIdxs = nil
}
